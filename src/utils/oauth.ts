...
export class OAuthHelper {
   ...
   */
  static async exchangeCodeForTokens(code: string, userId: string = 'default'): Promise<void> {
   ...
   */
  static async getAuthenticatedClient(userId: string = 'default'): Promise<typeof oauth2Client> {
   ...
   */
  static async isAuthenticated(userId: string = 'default'): Promise<boolean> {
    const tokenRecord = await db
      .select()
      .from(oauthTokens)
      .where(eq(oauthTokens.provider, 'google'))
