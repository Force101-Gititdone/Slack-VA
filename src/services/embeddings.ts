...
export class EmbeddingsService {
   ...
   */
  static async generateEmailEmbedding(emailId: string): Promise<void> {
    ...
    await db
      .update(emails)
      .set({
        contentEmbedding: embedding as any,
        updatedAt: new Date(),
      })
      .where(eq(emails.id, emailId));
  }
   ...
   */
  static async generateContactEmbedding(contactId: string): Promise<void> {
    ...
    await db
      .update(contacts)
      .set({
        profileEmbedding: embedding as any,
        updatedAt: new Date(),
      })
      .where(eq(contacts.id, contactId));
  }
   ...
   */
  static async batchGenerateEmailEmbeddings(limit: number = 100): Promise<number> {
   ...
   */
  static async batchGenerateContactEmbeddings(limit: number = 100): Promise<number> {
